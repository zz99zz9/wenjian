...
Call ShowCode("VerCode") '调用
Sub ShowCode(ShowCodeName)
	Set img = New fly38_VerifyCode
	Randomize
	Dim code
	code = Int(Rnd * 9000 + 1000)
	Session(ShowCodeName) = Trim(code)	'----session保存值------
	img.BGroundColor = "#A4BD44" ' 图片背景颜色
	img.FGroundColor = "#ffffff" ' 前景（文本）颜色
	Call img.VerifyCode(code, False)  ' 处理验证码，第二个参数为是否显示彩色文本。True:显示 False:不显示
	Call img.Noises(100, false)   ' 添加杂点，第一个参数为杂点数量，第二个参数为是否显示彩色杂点
	img.ImgWrite ' 输出图片
End Sub
